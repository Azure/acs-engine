...
for resourceGroup in `az group list | jq --arg dl $deadline '.[] | select(.id | contains("acse-test-infrastructure") or contains("acse-dnd") | not) | select(.tags.now < $dl).name' | tr -d '\"' || ""`; do

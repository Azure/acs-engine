...
var (
	cfg                    config.Config
	eng                    engine.Engine
	linuxClusterWaitTime   time.Duration
	windowsClusterWaitTime time.Duration
...
var _ = BeforeSuite(func() {
	...
	linuxClusterWaitTime = 10 * time.Minute
	windowsClusterWaitTime = 15 * time.Minute
	...
	if eng.HasWindowsAgents() {
		clusterWait = windowsClusterWaitTime
	} else {
		clusterWait = linuxClusterWaitTime
	}
})
...
var _ = Describe("Azure Container Cluster using the Kubernetes Orchestrator", func() {
	Context("regardless of agent pool type", func() {
		...
		It("should have kube-dns running", func() {
			running, err := pod.WaitOnReady("kube-dns", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})

		It("should have kube-dashboard running", func() {
			running, err := pod.WaitOnReady("kubernetes-dashboard", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})

		It("should have kube-proxy running", func() {
			running, err := pod.WaitOnReady("kube-proxy", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})

		It("should have heapster running", func() {
			running, err := pod.WaitOnReady("heapster", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})

		It("should have kube-addon-manager running", func() {
			running, err := pod.WaitOnReady("kube-addon-manager", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})

		It("should have kube-apiserver running", func() {
			running, err := pod.WaitOnReady("kube-apiserver", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})

		It("should have kube-controller-manager running", func() {
			running, err := pod.WaitOnReady("kube-controller-manager", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})

		It("should have kube-scheduler running", func() {
			running, err := pod.WaitOnReady("kube-scheduler", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})

		It("should have tiller running", func() {
			running, err := pod.WaitOnReady("tiller", "kube-system", 5*time.Second, clusterWait)
			Expect(err).NotTo(HaveOccurred())
			Expect(running).To(Equal(true))
		})
	...
	Context("with a linux agent pool", func() {
		It("should be able to deploy an nginx service", func() {
			if eng.HasLinuxAgents() {
				...
				running, err := pod.WaitOnReady(deploymentName, "default", 5*time.Second, clusterWait)
				Expect(err).NotTo(HaveOccurred())
				Expect(running).To(Equal(true))
				...
				Expect(err).NotTo(HaveOccurred())
				s, err = s.WaitForExternalIP(linuxClusterWaitTime, 5*time.Second)
				Expect(err).NotTo(HaveOccurred())
	...
	Context("with a windows agent pool", func() {
		It("should be able to deploy an iis webserver", func() {
			if eng.HasWindowsAgents() {
				...
				running, err := pod.WaitOnReady(deploymentName, "default", 5*time.Second, windowsClusterWaitTime)
				Expect(err).NotTo(HaveOccurred())
				Expect(running).To(Equal(true))
				...
				s, err := service.Get(deploymentName, "default")
				Expect(err).NotTo(HaveOccurred())
				s, err = s.WaitForExternalIP(windowsClusterWaitTime, 5*time.Second)
				Expect(err).NotTo(HaveOccurred())
				Expect(s.Status.LoadBalancer.Ingress).NotTo(BeEmpty())

...
var _ UpgradeWorkFlow = &Kubernetes171upgrader{}

// Kubernetes171upgrader upgrades a Kubernetes 1.6.6 cluster to 1.7.1
type Kubernetes171upgrader struct {
	ClusterTopology
...
func (ku *Kubernetes171upgrader) ClusterPreflightCheck() error {
	// Check that current cluster is 1.6.6
	if ku.DataModel.Properties.OrchestratorProfile.OrchestratorVersion != api.Kubernetes166 {
		return fmt.Errorf("Upgrade to Kubernetes 1.7.1 is not supported from version: %s",
...
func (ku *Kubernetes171upgrader) RunUpgrade() error {
	if err := ku.ClusterPreflightCheck(); err != nil {
		return err
	}

	ku.GoalStateDataModel = ku.ClusterTopology.DataModel
	ku.GoalStateDataModel.Properties.OrchestratorProfile.OrchestratorVersion = api.Kubernetes171
...
func (ku *Kubernetes171upgrader) Validate() error {
	return nil
}

func (ku *Kubernetes171upgrader) upgradeMasterNodes() error {
...
func (ku *Kubernetes171upgrader) upgradeAgentPools() error {
...
func (ku *Kubernetes171upgrader) generateUpgradeTemplate(upgradeContainerService *api.ContainerService) (map[string]interface{}, map[string]interface{}, error) {
	var err error

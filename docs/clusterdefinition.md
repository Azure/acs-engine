...
| clusterSubnet                   | no       | The IP subnet used for allocating IP addresses for pod network interfaces. The subnet must be in the VNET address space. The default value is dependent on Azure VNET intregration. VNET integration is enabled by default. With VNET integration enabled, the default value is 10.240.0.0/12. With VNET integration disabled, the default value is 10.244.0.0/16.                                            |

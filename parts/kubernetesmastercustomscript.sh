...
USE_MANAGED_IDENTITY_EXTENSION="${15}"

# Master only secrets
APISERVER_PRIVATE_KEY="${16}"
CA_CERTIFICATE="${17}"
CA_PRIVATE_KEY="${18}"
MASTER_FQDN="${19}"
KUBECONFIG_CERTIFICATE="${20}"
KUBECONFIG_KEY="${21}"
ADMINUSER="${22}"

# Default values for backoff configuration
CLOUDPROVIDER_BACKOFF="${23}"
CLOUDPROVIDER_BACKOFF_RETRIES="${24}"
CLOUDPROVIDER_BACKOFF_EXPONENT="${25}"
CLOUDPROVIDER_BACKOFF_DURATION="${26}"
CLOUDPROVIDER_BACKOFF_JITTER="${27}"
# Default values for rate limit configuration
CLOUDPROVIDER_RATELIMIT="${28}"
CLOUDPROVIDER_RATELIMIT_QPS="${29}"
CLOUDPROVIDER_RATELIMIT_BUCKET="${30}"

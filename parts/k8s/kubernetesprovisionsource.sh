...
systemctl_restart() { retries=$1; wait=$2; timeout=$3 svcname=$3; for i in $(seq 1 $retries); do timeout $timeout systemctl daemon-reload && systemctl restart $svcname && systemctl daemon-reload; [ $? -eq 0  ] && break || sleep $wait; done; }
...
function installMoby() {
    ...
    if [ $? -eq 0 ]; then
        ...
        retrycmd_if_failure 20 1 30 curl -L "https://azurek8scishared.blob.core.windows.net/shared/moby-engine_3.0.1_amd64.deb?sp=r&st=2018-10-15T22:20:13Z&se=2018-10-18T06:20:13Z&spr=https&sv=2017-11-09&sig=FB%2BmP4%2BmGmpctW4LXbseUYjPQNcgna5ROHDbcjxvPoM%3D&sr=b" > /tmp/moby-engine_3.0.1_amd64.deb
        retrycmd_if_failure 20 1 60 dpkg -i /tmp/moby-engine_3.0.1_amd64.deb
        apt_get_update || exit $ERR_APT_UPDATE_TIMEOUT

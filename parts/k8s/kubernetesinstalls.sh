...
installDeps() {
    retrycmd_if_failure_no_stats 120 5 25 curl -fsSL https://packages.microsoft.com/config/ubuntu/18.04/packages-microsoft-prod.deb > /tmp/packages-microsoft-prod.deb || exit $ERR_MS_PROD_DEB_DOWNLOAD_TIMEOUT
...
installGPUDrivers() {
    ...
    wait_for_apt_locks
    retrycmd_if_failure_no_stats 120 5 25 curl -fsSL https://nvidia.github.io/nvidia-docker/ubuntu18.04/amd64/nvidia-docker.list > /tmp/nvidia-docker.list || exit  $ERR_GPU_DRIVERS_INSTALL_TIMEOUT
    wait_for_apt_locks
...
installMoby() {
    ...
    if [ $? -eq 0 ]; then
        ...
        retrycmd_if_failure_no_stats 120 5 25 curl https://packages.microsoft.com/config/ubuntu/18.04/prod.list > /tmp/microsoft-prod.list || exit $ERR_MOBY_APT_LIST_TIMEOUT
...
installKataContainersRuntime() {
    ...
    KATA_URL=http://download.opensuse.org/repositories/home:/katacontainers:/release/xUbuntu_18.04/Release.key
    ...
    echo 'deb http://download.opensuse.org/repositories/home:/katacontainers:/release/xUbuntu_18.04/ /' > /etc/apt/sources.list.d/kata-containers.list
...
installClearContainersRuntime() {
    ...
    if [ $? -eq 0 ]; then
        ...
        CC_URL=https://download.opensuse.org/repositories/home:clearcontainers:clear-containers-3/xUbuntu_18.04/Release.key
        ...
        echo 'deb http://download.opensuse.org/repositories/home:/clearcontainers:/clear-containers-3/xUbuntu_18.04/ /' > /etc/apt/sources.list.d/cc-runtime.list

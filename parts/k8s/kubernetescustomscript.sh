...
#
# validateMountpoint( $DEVICE_PATH, $MOUNTPOINT )
#   return 0 if mount found
#   return 1 if mount is not reachable
...
function ensureEtcdDataDir() {
	...
	DEVICE_PATH="/dev/disk/azure/scsi1/lun0-part1"
	if [ -d "$MOUNTPOINT" ]; then
		for i in {1..60}; do
			if validateMountpoint $DEVICE_PATH $MOUNTPOINT; then
				echo "Found Mounted $DEVICE_PATH. Running with data dir at: $MOUNTPOINT"
				return
			else
				echo "$MOUNTPOINT was not found(expected at $DEVICE_PATH). Trying to wait and (re)mount all devices."
				sleep 5
			fi
		done
        exit 1
